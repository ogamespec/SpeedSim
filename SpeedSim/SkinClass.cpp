...
bool CSkinClass::Free()
{
	...
	while(it != m_Windows.end())
    {
        ...
        if(it->second.oldWndProc)
            SetWindowLongPtr(it->second.wnd, GWL_WNDPROC, (LONG)it->second.oldWndProc);
...
bool CSkinClass::AddWindow(HWND wnd, WNDTYPE type)
{
	...
	if(type == WT_BG)
	{
		oldWndProc = (WNDPROC)SetWindowLongPtr(wnd, GWL_WNDPROC, (long)SubClassProc);
	}
	if(type == WT_BTN)
	{
		// set to ownerdraw
	...
	if(type == WT_GROUPBOX)
	{
		...
		oldWndProc = (WNDPROC)SetWindowLongPtr(wnd, GWL_WNDPROC, (long)GroupBoxSubClassProc);
	}
	...
	{
        //DeactivateTheme(wnd, true);
		oldWndProc = (WNDPROC)SetWindowLongPtr(wnd, GWL_WNDPROC, (long)EditSubClassProc);
		w.BkColor = 0;
		w.BkMode = TRANSPARENT;
		w.TextColor = RGB(0, 0, 0);
	}
	if(type == WT_LABEL)
	{
		oldWndProc = (WNDPROC)SetWindowLongPtr(wnd, GWL_WNDPROC, (long)LabelSubClassProc);
        w.BkColor = 0;
		w.BkMode = TRANSPARENT;
		w.TextColor = RGB(0, 0, 0);
	}
